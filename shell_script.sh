# Our fixed base directory
BASE_DIR="/lab_1_sateliteDataAnalysis"

# Setting paths based on the fixed base directory
...
# Task 1: Extracting the top 10 highest temperatures
tail -n +2 "$DATAFILE" | sort -t',' -k4 -nr | head -10 > "$OUTPUT_TOP10"
head -n 1 "$DATAFILE" | cat - "$OUTPUT_TOP10" > temp && mv temp "$OUTPUT_TOP10"

# Task 2: Extracting data for selected country, sorted by humidity descending
